...
def connect_to_chroma(chroma_host:str=None, chroma_port:int=None, local:bool=True):
    '''
    DESCRIPTION
    -----------
...
def connect_to_collection(client:chromadb.HttpClient, 
                          collection:str, 
                          metadata:dict={"hnsw:space": "cosine"},
                          embedding_function=None,
                          overwrite:bool=False) -> chromadb.Collection:
    '''
    DESCRIPTION
    -----------
    ...
    return client.get_or_create_collection(name=collection, metadata=metadata, embedding_function=embedding_function)
...
def format_data(data:List[Document], ids:list=None, max_token:int=8191) -> Tuple[Dict[str, list],list]:
    '''
    DESCRIPTION
    -----------
    ...
    documents_below, documents_above = parse_data_by_limit(documents, indices_below, indices_above)
    
    # separate metadatas
    metadatas = [d.metadata for d in data]
    metadatas_below, metadatas_above = parse_data_by_limit(metadatas, indices_below, indices_above)

    ids = generate_uuids(len(documents_below)) if ids is None else ids
    data_to_add = {"documents": list(documents_below), "metadatas": list(metadatas_below), "ids": ids}
...
def upload_to_collection(collection:chromadb.Collection,
                         data:List[Document],
